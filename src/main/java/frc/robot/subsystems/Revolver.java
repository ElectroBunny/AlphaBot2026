...
public class Revolver extends SubsystemBase 
{
  ...
  private Revolver() {
    motor = new SparkFlex(Constants.REVOLVER_MOTOR_ID, MotorType.kBrushless);
    ...
    motorConfig.idleMode(IdleMode.kBrake);
		motorConfig.smartCurrentLimit(Constants.REVOLVER_MOTOR_CURRENT_LIMIT);
		motor.configure(motorConfig, ResetMode.kResetSafeParameters, PersistMode.kNoPersistParameters);
    		motorConfig.closedLoop
				.feedbackSensor(FeedbackSensor.kPrimaryEncoder)
				.p(Constants.REVOLVER_P)
				.i(Constants.REVOLVER_I)
				.d(Constants.REVOLVER_D)
				.outputRange(-1, 1);
  ...
  {
    return (Math.abs(motorEncoder.getVelocity() - velocity) <= Constants.REVOLVER_VELOCITY_TOLERANCE);
  }
